...
let inputFile = path.join(buildPath, "gen", "lib", "lib.deno_runtime.d.ts");
let outputFile = path.join(buildPath, "gen", "docs", "deno_runtime.json");
let silent = false;
...
process.argv.forEach((arg, i, argv) => {
  ...
  switch (arg) {
    ...
    case "--buildPath":
      ...
      break;
    case "--inputFile":
      inputFile = path.resolve(argv[i + 1]);
      break;
    case "--outputFile":
      outputFile = path.resolve(argv[i + 1]);
      break;
    case "--silent":
      silent = true;
      break;
...
const generateJsonDocs = require("./generate_json_docs").main;
