...
fn op_code_fetch(
  ...
  assert_eq!(
    state.code_provider.root.join("gen"),
    state.code_provider.gen,
    "Sanity check"
  );
...
fn op_code_cache(
  ...
  Box::new(futures::future::result(|| -> OpResult {
    state.code_provider.code_cache(
      filename,
      source_code,
      output_code,
      source_map,
...
fn op_repl_start(
  ...
  let history_path = repl::history_path(&state.code_provider, &history_file);
